...
if [[ -z "${SCHEDULE:-}" ]]; then
  echo "SCHEDULE env var is required (cron syntax)."
  exit 2
fi

if [[ -z "${EXPOSITION_ENDPOINT:-}" ]]; then
  echo "EXPOSITION_ENDPOINT env var is required (full URL for POSTing Prometheus exposition data)."
  exit 2
fi
